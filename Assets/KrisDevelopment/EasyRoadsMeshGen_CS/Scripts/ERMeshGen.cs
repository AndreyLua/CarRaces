...
namespace KrisDevelopment.ERMG
{
	...
	public sealed class ERMeshGen : MonoBehaviour
	{
		private class TerrainStrokeInfo
		{
			...
			public float heightTexelWorldSizeX;
			public float heightTexelWorldSizeY;
		...
		public void UpdateMesh(DisposeMethod dispose = null)
		{
			...
			ForExtensions(a => { if(!a.suspend) a.UpdateState(this, dispose ?? Dispose); });
			onUpdateMesh?.Invoke();
		}
		...
		private void SetVerts()
		{
			...
			for (int n = 0; n < pathPoints.Length; n++)
			{
				if (legacyWidthMode)
				{
					...
					var _roadEdge = _pathPoint.right * _pathPoint.width; // offset to the edge of the road based on road width

                    vertices[n * 2] = _worldToLocalMatrix.MultiplyPoint3x4(_pathPoint.position - _roadEdge);
                    vertices[n * 2 + 1] = _worldToLocalMatrix.MultiplyPoint3x4(_pathPoint.position + _roadEdge);
                    ...
                    for (int b = 0; b < borderCurve.length; b++)
					{
						if (borderCurve.length + b < rightBorderVertices.Length)
						{
							...
							var _right = _roadEdge + _pathPoint.right * halfDeltaWidth * borderSectionPoints[b].x;
							
                            rightBorderVertices[n * borderCurve.length + b] = _worldToLocalMatrix.MultiplyPoint3x4(_pathPoint.position + _right + _up);
							leftBorderVertices[n * borderCurve.length + b] = _worldToLocalMatrix.MultiplyPoint3x4(_pathPoint.position - _right + _up);
						}
					}
				}
				else
				{
                    ...
                    for (int b = 0; b < borderCurve.length; b++)
					{
						...
						{
							var _up = _pathPoint.up * borderSectionPoints[b].y;
							var _right = _roadEdge + _pathPoint.right * borderSectionPoints[b].x;
		...
		public void ProcessUnderlyingTerrains()
		{
			...
			{
				MorphTerrain(_terrain, _progress++, string.Format("Processing Terrain {0}/{1}", _progress, _terrains.Count));
			}
		}

		public void MorphTerrain(Terrain terrain, int terrainIndex, string progressMessage = "")
		{
#if UNITY_EDITOR
			...
			var _heightTexelWorldSizeX = _terrainSize.x / _heightmapWidth;
			var _heightTexelWorldSizeY = _terrainSize.z / _heightmapHeight;

			var _terrainStrokeInfo = new TerrainStrokeInfo()
			{
				terrainSize = _terrainSize,
				heightTexelWorldSizeX = _heightTexelWorldSizeX,
				heightTexelWorldSizeY = _heightTexelWorldSizeY,
				...
				brushSamplesX = (int)(_totalRadius / _heightTexelWorldSizeX),
				brushSamplesY = (int)(_totalRadius / _heightTexelWorldSizeY),
			...
			for (int i = 0; i < _pathPoints.Length - 1; i++)
			{
				...
				{
					int _strokeHeightmapCenterX = (int)((_strokePosition.x - _terrainPosition.x) / _terrainStrokeInfo.heightTexelWorldSizeX);
					int _strokeHeightmapCenterY = (int)((_strokePosition.z - _terrainPosition.z) / _terrainStrokeInfo.heightTexelWorldSizeY);
		...
		private void ApplyBrushOnTerrainHeight(ref float[,] heights, int centerX, int centerY, TerrainStrokeInfo strokeInfo, Vector3 strokePosition)
		{
			...
			for (int x = -strokeInfo.brushSamplesX; x < strokeInfo.brushSamplesX * 2; x++)
			{
				for (int y = -strokeInfo.brushSamplesY; y < strokeInfo.brushSamplesY * 2; y++)
				{
					...
					var _point3f = new Vector3(x * strokeInfo.heightTexelWorldSizeX, _currentRealTerrainHeight, y * strokeInfo.heightTexelWorldSizeY) + strokePosition;
        ...
        private void DrawDebugPosition(Vector3 strokePosition, Color? color = null)
		{
			Debug.DrawLine(strokePosition + Vector3.forward, strokePosition - Vector3.forward, color ?? Color.red, 1f);
			Debug.DrawLine(strokePosition + Vector3.right, strokePosition - Vector3.right, color ?? Color.red, 1f);
		}
		...
		/// </summary>
		public void RuntimeFinalize(DisposeMethod dispose)
		{
			Finalize(dispose, (o) => { });
        }

		/// <summary>
		/// Common finalize method for both Editor and Runtime use.
		/// </summary>
        private void Finalize(DisposeMethod dispose, AssetCreationMethod assetCreation)
        {
		...
		public void EditorFinalize(DisposeMethod dispose)
		{
			...
			Finalize(dispose, (GameObject o) => { EditorFinalizeERObject(o, _genericPiecePath); });

			Util.Dirtify(this);
		}

		/// <summary>
		/// Exports assets. You can provide this method as an AssetCreationMethod.
		/// See <see cref="Finalize"/>
		/// </summary>
		...
		private void UpdateCollider(Mesh colMesh)
		{
			if (includeCollider == 1)
			{
				var _enoughPointsPresent = navPoints.Count > 1;
				if (!_enoughPointsPresent)
				{
					return;
				}

#if UNITY_EDITOR
				if (colMesh.vertices.Length < 3 && _enoughPointsPresent)
				{
					var _assertContextInfo = new StringBuilder();
					_assertContextInfo.AppendLine($".Scene: {string.Join(", ", UnityEngine.SceneManagement.SceneManager.GetActiveScene().name)}");
					_assertContextInfo.AppendLine($".GameObject: {name}");
					_assertContextInfo.AppendLine($".Components: {string.Join(",", GetComponents<Component>().Select(a => a.name))}");
					_assertContextInfo.AppendLine($".Mesh: -name {colMesh.name}, -verts {colMesh.vertexCount}, -tris {colMesh.triangles.Length}");
					_assertContextInfo.AppendLine($".Nav Points: {navPoints.Count}");

					throw new Exception( $"[ERMG] Collision Mesh must have at least 3 vertices. {_assertContextInfo}");
				}
#endif
		...
		/// <summary>
		/// Returns data about the nav point transforms. This allocates memory.
		/// </summary>
