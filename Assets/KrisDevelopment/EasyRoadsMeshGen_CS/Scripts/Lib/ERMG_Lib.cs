...
namespace KrisDevelopment.ERMG
{
	...
	{
        private static Material _defaultMaterialCached;
		...
		{
			if (_defaultMaterialCached != null)
			{
				return _defaultMaterialCached;
			}

			var _sample = GameObject.CreatePrimitive(PrimitiveType.Quad);
			_sample.hideFlags = HideFlags.DontSave;
			_defaultMaterialCached = _sample.GetComponent<Renderer>().sharedMaterial;
