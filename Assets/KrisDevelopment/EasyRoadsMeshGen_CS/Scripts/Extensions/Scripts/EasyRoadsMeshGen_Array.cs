...
namespace KrisDevelopment.ERMG
{
    ...
    public class EasyRoadsMeshGen_Array : MeshGenExtensionComponent
    {
        ...
        public class ArrayObject
        {
            ...
            [Tooltip("This is how much the end element should stick out of the road. Value of 1 will always spawn one additional element at the end.")]
            [Range(0,1)]
            public float lastElementExtensionThreshold = 0;
        ...
        ///</summary>
        private static int CalculateFit(FitType fitType, float objectLength, ERPathTracer path, float predefinedLength, float extension)
        {
            //set the AMOUNT variable based on fit length
            if (fitType == FitType.FitPath)
            {
                var _fitLength = path.distanceRecord + (extension * objectLength);
            ...
            {
                int _fitAmount = (int)Mathf.Floor(predefinedLength / objectLength + (extension * objectLength));
                return _fitAmount;
            }
            ...
            if (arrayObjects != null)
            {
                for (int i = 0; i < arrayObjects.Count; i++)
                {
                    ...
                    if (arrayObject.fitType != FitType.FixedAmount)
                        arrayObject.amount = CalculateFit(arrayObject.fitType, arrayObject.length, _path, length, arrayObject.lastElementExtensionThreshold);
        ...
        public override void OnFinalize(ERMeshGen meshGen, DisposeMethod dispose, AssetCreationMethod assetCreation = null)
        {
            ...
            foreach (var _er in GetComponentsInChildren<ERArrayObjectInstance>())
            {
                dispose(_er);
            }
